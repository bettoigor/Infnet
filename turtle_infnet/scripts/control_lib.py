...
def polar_control(robot_pose,goal,K_rho,K_alpha, K_beta):
    ...
    rho  = round(math.sqrt(delta_x**2 + delta_y**2),5)
    alpha = round(angles.shortest_angular_distance(theta,heading),5)
    beta = round(-theta - alpha + theta_d,5)
    ...
    v = round(K_rho*rho*math.cos(alpha),3)
